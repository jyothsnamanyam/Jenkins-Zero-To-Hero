...
def internet_route(ec2, one_tgw, DryRun=None):
    ...
    for route_table in route_table_list:
        ...
        replace_route_one_tgw(ec2, route_table, one_tgw, DryRun=DryRun)
...
def intranet_route(ec2, one_tgw, DryRun=None):
    ##Getting all vpcs routetable ids in that account by region.
    route_table_list = get_routetables(ec2)
    for route_table in route_table_list:
...
def spoke_account_route_create(account_type, sts, arn, region_name, one_tgw, DryRun):
    if account_type == "INTRANET":
        target_account_session = target_session(sts, arn)
        ec2 = target_account_session.client("ec2", region_name)
        intranet_route(ec2, one_tgw, DryRun=DryRun)
    elif account_type == "INTERNET":
        target_account_session = target_session(sts, arn)
        ec2 = target_account_session.client("ec2", region_name)
        internet_route(ec2, one_tgw, DryRun=DryRun)
...
def main(env_type, region_name, account_type, access_type):
    global pl_id
    if env_type == "PROD":
        from gb_tgw_constants import TGW_CONSTANTS, DYNAMODB_TABLE_ACCOUNT_DATA
    else:
        from tst_tgw_constants import TGW_CONSTANTS, DYNAMODB_TABLE_ACCOUNT_DATA
    ...
    if access_type == "IDY_ROLE_KEYS":
        ...
        """
        arn = "arn:aws:iam::866919043554:role/RRCC_AWS_ONEDESIGNTESTING"
        ...
        for arn in arn_list:
            logger.info(f"Create Routing rules in Account: {arn}")
            spoke_account_route_create(account_type, sts, arn, region_name, ONE_TGW, DryRun=True)
            # Thread(target=spoke_account_route_create, args=(account_type, sts, arn, region_name, ONE_TGW, DMZ_TGW, SAME_CROSS_CIDRS_ENTRIES[region_name] )).start()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog="python3 scripts/phase02_default_route.py --env_type='TEST' --region_name='ap-northeast-1' --account_type='INTRANET' --access_type='IDY_ROLE_KEYS'"
    )
