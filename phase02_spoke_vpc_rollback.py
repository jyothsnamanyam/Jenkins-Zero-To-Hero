...
def internet_route(ec2, dmz_tgw, DryRun=None):
    ...
    for route_table in route_table_list:
        #replacing the default from ONE TGW to DMZ TGW.
...
def intranet_route(ec2, bst_tgw, DryRun=None):
    route_table_list = get_routetables(ec2)
    for route_table in route_table_list:
        ##replacing the default from  ONE TGW to BST TGW.
        logger.info(f"Updating the Route Table: {route_table}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog="python3 scripts/phase02_spoke_vpc_rollback.py --env_type='TEST' --region_name='ap-northeast-1' --account_type='INTRANET' --access_type='IDY_ROLE_KEYS'"
    )
