...
pipeline {
    ...
    stages {
        ...
        stage('DMZ NAT VPC SPOKE AUTOMATION') {
            steps {
                script {
                    sh '''
                    python3 scripts/dmz_nat_vpc_rollback.py --env_type=$NVS_ENV_TYPE --region_name=$RegionName
                    '''
                }
            }
        }
    ...
    post {
        success {
            script {
                println "DMZ NAT Spoke VPCs routes update was success."
                ...
                sh '''
                echo "Make sure you remove the DMZ VPC CIDR from BGP Import filter in EWTP IPSEC-VPN"
                '''
            }
        }
        failure {
            script {
                println "DMZ, NAT Spoke VPC routes update was failed."
            }
        }
    }
